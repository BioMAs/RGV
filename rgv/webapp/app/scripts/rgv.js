...
config(['$routeProvider','$logProvider',
    function ($routeProvider) {
        ...
        $routeProvider.when('/browser_density', {
            templateUrl: 'views/density_plot.html',
            controller: 'browser_densityCtrl'
        });
...
angular.module('rgv').controller('downloadCtrl',
    function ($scope,$rootScope, $log, Auth, User, Dataset, $q, $filter) {
        ...
        $scope.haveOnePath = function(path){
            ...
            if (path.indexOf('|') > -1){
                ...
                for (var i=0;i<finalString.length;i++){
                    var dico = {};
                    dico.path = finalString[i].split("/").join('+');
                    dico.path = dico.path +'.zip';
